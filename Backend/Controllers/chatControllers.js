...
export const accesschat = async (req, res) => {
  ...
  isChat-await userModel.populate(isChat,{path:'latestMessage.sender',sender:"name pic email"})
  if(isChat.length>0){
    res.send(isChat[0])
  }else{
    var chatData={
...
if(users.length<2){
return res
.status(404).send({message:"More than 2 users are required to form group chat"})
}
users.push(req.user)
try{
